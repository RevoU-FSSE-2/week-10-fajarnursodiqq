...
const deleteTransfer = async (req, res) => {
  ...
  try {
    ...
    if (transfer.status !== "pending") {
      res.status(403).json({ message: "Only pending transfer can be deleted" });
      return;
    }
    ...
    res
      .status(200)
      .json({ message: "Transfer temporary deleted successfully" });
  } catch (error) {
...
const getTransferHistory = async (req, res) => {
  try {
    ...
    const transfer = await req.db.collection("transfer").find(query).toArray();
    res.status(200).json(transfer);
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
